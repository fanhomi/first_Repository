...
const {query} = require("./importEWE")
const {CRUDdatabase,HandleReturn,FlowDiagram} = require("./exportEWE")
...
exports.R_Ecopath = (req,res)=>{
    ...
    const InsertGroup = "INSERT INTO EcopathGroup3(GroupName,Type,Biomass,ProdBiom,EcoEfficiency,ProdCons,BiomAcc,Unassim) VALUES "
    ...
    pool.connect().then((client) => {
        ...
        // 创建Diet表
            ...
            // 插入Group数据
            client.query(InsertGroup+sqlString.substring(0,sqlString.length-1),GroupData, (err, res) => {
                if (err) {console.log(err)};
                // client.release()
            })
          }).then(()=>{
            // 插入Diet数据
            client.query(InsertDiet+sqlStrDiet.substring(0,sqlStrDiet.length-1),sqlDiet, (err, res) => {
                if (err) {console.log(err)};
                client.release()
            })
          })
...
exports.R_test2 = (req,res)=>{
    ...
    database.then(()=>{
        cs.exec(`Rscript ./src/model/R/EcoPath.R ${num}` ,(error,stdout,stderr)=>{
            if (error) {
                console.error('error:', error);
            }
            ...
            // res.send(data)
            res.send({BasicEst:HandleReturn(data.Basic,data.InputFlag),Graph:FlowDiagram(data.prenode,data.link)})
            // res.send(data)
        })
...
exports.R_test3 = (req,res)=>{
    ...
    const connection = ADODB.open(`Provider=Microsoft.ACE.OLEDB.12.0;Data Source=./${req.file.path};Persist Security Info=False;`);
